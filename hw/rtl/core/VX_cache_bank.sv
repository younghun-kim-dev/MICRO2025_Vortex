...
module VX_cache_bank import VX_gpu_pkg::*; #(
    ...
    // NOTE: TAG layout is assumed to be { PREFETCH[MSB], UUID[UUID_WIDTH-1:0] }
...
`IGNORE_UNUSED_END
    ...
    // ===== Prefetch 비트(태그 MSB) 추출 =====
    wire                            is_prefetch_sel;
    wire                            is_prefetch_st0;
    wire                            is_prefetch_st1;
    // ======================================
    ...
    if (UUID_WIDTH != 0) begin : g_flush_tag_uuid
        // TAG layout: {prefetch_bit, uuid}
        // For flush_tag we set prefetch_bit to 0 and keep uuid = flush_uuid
        assign flush_tag = {1'b0, flush_uuid};
    ...
    // ===== Prefetch 비트: TAG MSB =====
    assign is_prefetch_sel = tag_sel[TAG_WIDTH-1];
    // ==================================
    ...
    // UUID 추출: TAG MSB(prefetch) 건너뛰고 UUID만 사용
    if (UUID_WIDTH != 0) begin : g_req_uuid_sel
        assign req_uuid_sel = tag_sel[TAG_WIDTH-2 -: UUID_WIDTH];
    ...
    VX_pipe_register #(
        .DATAW  (1 + 1 + 1 + 1 + 1 + 1 + `UP(MEM_FLAGS_WIDTH) + `CS_WAY_SEL_WIDTH + `CS_LINE_ADDR_WIDTH + `CS_LINE_WIDTH + 1 + WORD_SIZE + WORD_SEL_WIDTH + REQ_SEL_WIDTH + TAG_WIDTH + MSHR_ADDR_WIDTH + 1),
        .RESETW (1)
    ) pipe_reg0 (
        .clk      (clk),
        .reset    (reset),
        .enable   (~pipe_stall),
        .data_in  ({valid_sel, is_init_sel, is_fill_sel, is_flush_sel, is_creq_sel, is_replay_sel, flags_sel, flush_way,     addr_sel, data_sel, rw_sel, byteen_sel, word_idx_sel, req_idx_sel, tag_sel, replay_id, is_prefetch_sel}),
        .data_out ({valid_st0, is_init_st0, is_fill_st0, is_flush_st0, is_creq_st0, is_replay_st0, flags_st0, flush_way_st0, addr_st0, data_st0, rw_st0, byteen_st0, word_idx_st0, req_idx_st0, tag_st0, replay_id_st0, is_prefetch_st0})
    );

    // UUID (prefetch 비트 제외)
    if (UUID_WIDTH != 0) begin : g_req_uuid_st0
        assign req_uuid_st0 = tag_st0[TAG_WIDTH-2 -: UUID_WIDTH];
    ...
    VX_pipe_register #(
        .DATAW  (1 + 1 + 1 + 1 + 1 + 1 + 1 + 1 + `UP(MEM_FLAGS_WIDTH) + `CS_WAY_SEL_WIDTH + `CS_TAG_SEL_BITS + `CS_TAG_SEL_BITS + `CS_LINE_SEL_BITS + `CS_LINE_WIDTH + WORD_SIZE + WORD_SEL_WIDTH + REQ_SEL_WIDTH + TAG_WIDTH + MSHR_ADDR_WIDTH + MSHR_ADDR_WIDTH + 1 + 1),
        .RESETW (1)
    ) pipe_reg1 (
        .clk      (clk),
        .reset    (reset),
        .enable   (~pipe_stall),
        .data_in  ({valid_st0, is_fill_st0, is_flush_st0, is_creq_st0, is_replay_st0, is_dirty_st0, is_hit_st0, rw_st0, flags_st0, way_idx_st0, evict_tag_st0, line_tag_st0, line_idx_st0, data_st0, byteen_st0, word_idx_st0, req_idx_st0, tag_st0, mshr_id_st0, mshr_previd_st0, mshr_pending_st0, is_prefetch_st0}),
        .data_out ({valid_st1, is_fill_st1, is_flush_st1, is_creq_st1, is_replay_st1, is_dirty_st1, is_hit_st1, rw_st1, flags_st1, way_idx_st1, evict_tag_st1, line_tag_st1, line_idx_st1, data_st1, byteen_st1, word_idx_st1, req_idx_st1, tag_st1, mshr_id_st1, mshr_previd_st1, mshr_pending_st1, is_prefetch_st1})
    );

    // UUID (prefetch 비트 제외)
    if (UUID_WIDTH != 0) begin : g_req_uuid_st1
        assign req_uuid_st1 = tag_st1[TAG_WIDTH-2 -: UUID_WIDTH];
    ...
    ///////////////////////////////////////////////////////////////////////////////
    // Prefetch 메타데이터 & 카운터(펄스) 로직
    //  - MSHR 엔트리별 prefetch 표시: prefetch_id[i]
    //  - 라인/웨이 메타: was_prefetched[way][line], used[way][line]
    //  - 이벤트 펄스:
    //      * pf_unique_w      : 유니크 프리패치 메모리요청
    //      * pf_unused_w      : 미사용 프리패치 라인(에빅 시 판정)
    //      * pf_late_w        : 레이트 프리패치(프리패치 pending 중 demand 도착)
    ///////////////////////////////////////////////////////////////////////////////

    // MSHR id 별로 "이 엔트리가 프리패치인가?" 저장
    reg [MSHR_SIZE-1:0] prefetch_id;
    // 라인/웨이 메타
    reg [NUM_WAYS-1:0][`CS_LINES_PER_BANK-1:0] meta_was_prefetched;
    reg [NUM_WAYS-1:0][`CS_LINES_PER_BANK-1:0] meta_used;

    // allocate 시 프리패치 엔트리 마킹 (st0에서 판단)
    wire alloc_is_prefetch = is_prefetch_st0;
    // dequeue/릴리즈 시 클리어
    wire replay_fire_w = replay_fire; // dequeue된 id는 replay_id
    // fill 발생 시, 해당 fill을 시작한 엔트리가 프리패치였는지
    wire fill_is_prefetch = prefetch_id[mem_rsp_id];

    // 레이트 프리패치: 기존 pending 엔트리(같은 주소)의 꼬리(prev)가 프리패치이고,
    // 지금 들어온 건 demand(비-프리패치)일 때 카운트
    wire late_prefetch_w = (mshr_allocate_st0 && ~is_prefetch_st0 && ~rw_st0  // demand(read) allocate
                         &&  mshr_pending_st0
                         &&  prefetch_id[mshr_previd_st0]);

    // 유니크 프리패치 메모리 요청: ST1에서 mreq_queue_push가 프리패치 read-miss로 인해 발생
    ...
    if (WRITE_ENABLE) begin : g_mreq_queue
        if (WRITEBACK) begin : g_wb
            ...
            // 유니크 프리패치(= 프리패치 read-miss가 메모리로 감)
            assign pf_unique_w = mreq_queue_push && ~is_fill_or_flush_st1 && ~rw_st1 && is_prefetch_st1;
            ...
            // 유니크 프리패치(= 프리패치 read-miss가 메모리로 감)
            assign pf_unique_w = mreq_queue_push && ~rw_st1 && is_prefetch_st1;
        ...
        // 유니크 프리패치
        assign pf_unique_w = mreq_queue_push && is_prefetch_st1;
    ...
    // ====== 메타데이터/카운터 업데이트 ======
    // - init/flush 시 해당 line의 used/was_prefetched 초기화
    // - fill(st1)에서 에빅되는 라인 unused 판정 후 카운트
    // - fill(st0)에서 새 라인 메타 기록(was_prefetched=fill_is_prefetch, used=0)
    // - demand hit 시 used=1
    // - MSHR allocate/dequeue/release에 따라 prefetch_id[] 유지
    // ======================================

    // 에빅되는 라인에서 "미사용 프리패치" 검출
    wire evict_was_prefetched  = meta_was_prefetched[way_idx_st1][line_idx_st1];
    wire evict_used            = meta_used[way_idx_st1][line_idx_st1];
    wire pf_unused_w           = (valid_st1 && is_fill_st1 && (evict_was_prefetched && ~evict_used));
    ...
    always @(posedge clk) begin
        if (reset) begin
            ...
            for (wi = 0; wi < NUM_WAYS; wi = wi + 1) begin
                meta_was_prefetched[wi] <= '0;
                meta_used[wi]           <= '0;
            end
        end else begin
            // MSHR 엔트리 프리패치 표기
            if (mshr_allocate_st0 && ~pipe_stall) begin
                // allocate는 항상 일어나므로, 프리패치 여부만 기록
                prefetch_id[mshr_alloc_id_st0] <= alloc_is_prefetch;
            end
            if (replay_fire_w) begin
                prefetch_id[replay_id] <= 1'b0; // dequeue된 엔트리는 제거
            end
            if (mshr_release_fire) begin
                prefetch_id[mshr_id_st1] <= 1'b0; // hit 등으로 릴리즈된 엔트리 제거
            end

            // init: 해당 line 모든 way 초기화
            if (do_init_st0) begin
                for (wi = 0; wi < NUM_WAYS; wi = wi + 1) begin
                    meta_was_prefetched[wi][line_idx_st0] <= 1'b0;
                    meta_used[wi][line_idx_st0]           <= 1'b0;
                end
            end

            // flush: writethrough에서는 모든 way, writeback에서는 선택 way
            if (do_flush_st0 && ~pipe_stall) begin
                if (WRITEBACK) begin
                    meta_was_prefetched[evict_way_st0][line_idx_st0] <= 1'b0;
                    meta_used[evict_way_st0][line_idx_st0]           <= 1'b0;
                end else begin
                    for (wi = 0; wi < NUM_WAYS; wi = wi + 1) begin
                        meta_was_prefetched[wi][line_idx_st0] <= 1'b0;
                        meta_used[wi][line_idx_st0]           <= 1'b0;
            ...
            // fill(st1): 에빅되는 라인의 unused 판정은 pf_unused_w 펄스로 별도 카운트 (상단에서 생성)

            // fill(st0): 새로 채워지는 라인의 메타 기록
            if (do_fill_st0 && ~pipe_stall) begin
                meta_was_prefetched[way_idx_st0][line_idx_st0] <= fill_is_prefetch;
                meta_used[way_idx_st0][line_idx_st0]           <= 1'b0;
            end

            // demand hit → used=1
            if (do_read_st1 && is_hit_st1 && ~is_prefetch_st1 && ~pipe_stall) begin
                meta_used[way_idx_st1][line_idx_st1] <= 1'b1;
            end
            // write hit도 사용으로 간주하려면 아래 주석 해제
            if (do_write_st1 && is_hit_st1 && ~is_prefetch_st1 && ~pipe_stall) begin
                meta_used[way_idx_st1][line_idx_st1] <= 1'b1;
            end
        end
    end
...
`ifdef PERF_ENABLE
    ...
    // (내부) 프리패치 과제용 이벤트 펄스:
    //  - pf_unique_w : 유니크 프리패치 메모리 요청
    //  - pf_unused_w : 미사용 프리패치 에빅
    //  - late_prefetch_w : 레이트 프리패치
    // 외부 포트 변경 없이, 상위 퍼프레임이 pulsed event를 집계하도록
    // 필요시 여기서 $display로 찍어볼 수도 있음.
`endif

`ifdef DBG_TRACE_CACHE
    // Prefetch 비트 (MSB) 확인
    wire core_req_is_prefetch = core_req_tag[TAG_WIDTH-1];
    wire st1_is_prefetch_dbg  = is_prefetch_st1;
    ...
    always @(posedge clk) begin
        ...
        if (mem_rsp_fire) begin
            `TRACE(2, ("%t: %s fill-rsp: addr=0x%0h, mshr_id=%0d, data=0x%h, fill_pf=%b (#%0d)\n", $time, INSTANCE_ID,
                mem_rsp_full_addr, mem_rsp_id, mem_rsp_data, fill_is_prefetch, req_uuid_sel))
        end
        ...
        if (core_req_fire) begin
            if (core_req_rw) begin
                `TRACE(2, ("%t: %s core-wr-req: addr=0x%0h, tag=0x%0h, req_idx=%0d, byteen=0x%h, data=0x%h, pref=%b (#%0d)\n", $time, INSTANCE_ID,
                    core_req_full_addr, core_req_tag, core_req_idx, core_req_byteen, core_req_data, core_req_is_prefetch, req_uuid_sel))
            end else begin
                `TRACE(2, ("%t: %s core-rd-req: addr=0x%0h, tag=0x%0h, req_idx=%0d, pref=%b (#%0d)\n", $time, INSTANCE_ID,
                    core_req_full_addr, core_req_tag, core_req_idx, core_req_is_prefetch, req_uuid_sel))
            end
        end
        if (do_init_st0) begin
        ...
        if (do_fill_st0 && ~pipe_stall) begin
            `TRACE(3, ("%t: %s tags-fill(new): addr=0x%0h, way=%0d, line=%0d, dirty=%b, was_pf=%b (#%0d)\n", $time, INSTANCE_ID,
                full_addr_st0, way_idx_st0, line_idx_st0, is_dirty_st0, fill_is_prefetch, req_uuid_st0))
        end
        if (do_flush_st0 && ~pipe_stall) begin
        ...
        if (do_read_st1 && is_hit_st1 && ~pipe_stall) begin
            `TRACE(3, ("%t: %s data-read: addr=0x%0h, way=%0d, line=%0d, wsel=%0d, data=0x%h, pref(st1)=%b (#%0d)\n", $time, INSTANCE_ID,
                full_addr_st1, way_idx_st1, line_idx_st1, word_idx_st1, crsp_queue_data, st1_is_prefetch_dbg, req_uuid_st1))
        end
        if (do_write_st1 && is_hit_st1 && ~pipe_stall) begin
            `TRACE(3, ("%t: %s data-write: addr=0x%0h, way=%0d, line=%0d, wsel=%0d, byteen=0x%h, data=0x%h, pref(st1)=%b (#%0d)\n", $time, INSTANCE_ID,
                full_addr_st1, way_idx_st1, line_idx_st1, word_idx_st1, byteen_st1, write_word_st1, st1_is_prefetch_dbg, req_uuid_st1))
        end
        ...
        if (mreq_queue_push) begin
            if (!WRITEBACK && do_write_st1) begin
                ...
                `TRACE(2, ("%t: %s fill-req: addr=0x%0h, mshr_id=%0d, pref(st1)=%b, pf_unique=%b (#%0d)\n", $time, INSTANCE_ID,
                    mreq_queue_full_addr, mshr_id_st1, st1_is_prefetch_dbg, pf_unique_w, req_uuid_st1))
            end
        end
        if (pf_unused_w) begin
            `TRACE(2, ("%t: %s pf-unused-evict: addr=0x%0h, way=%0d, line=%0d (was_pf=1, used=0)\n", $time, INSTANCE_ID,
                `CS_BANK_TO_FULL_ADDR({evict_tag_st1, line_idx_st1}, BANK_ID), way_idx_st1, line_idx_st1))
        end
        if (late_prefetch_w) begin
            `TRACE(2, ("%t: %s late-prefetch: addr=0x%0h (pending pf MSHR id=%0d) (#%0d)\n", $time, INSTANCE_ID,
                full_addr_st0, mshr_previd_st0, req_uuid_st0))
        end
    end
`endif
