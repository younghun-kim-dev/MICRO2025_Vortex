        ...
        void processRequests() {
                ...
                case bank_req_t::Core: {
                        ...
                        if (hit_line_id != -1) {
                                ...
                                // ADD: Mark as used if it was prefetched and this is a prefetch access
                                ...
                                // CHANGE: send core response (skip for prefetch)
                                ...
                                // ADD: Counter 1 - unique prefetch requests that miss
                                if (bank_req.is_prefetch) {
                                        ++perf_stats_.prefetch_requests;
                                }

                                // ADD: check MSHR pending for same address
                                ...
                                if (free_line_id == -1 && config_.write_back) {
                                        ...
                                        // ADD: Counter 2 - unused prefetch eviction
                                ...
                                if (bank_req.write && !config_.write_back) {
                                        ...
                                        // send fill request (only if not already pending)
                                        if (!mshr_pending) {
                                                MemReq mem_req;
                                                mem_req.addr  = params_.mem_addr(bank_id_, bank_req.set_id, bank_req.addr_tag);
                                                mem_req.write = false;
