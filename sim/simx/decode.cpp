...
static op_string_t op_string(const Instr &instr) {
  ...
  return visit_var(op_type,
    [&](AluType alu_type)-> op_string_t {
      ...
      case AluType::DOT8:  return {"DOT8", ""}; // DOT8
      default:
        std::abort();
      }
    },
...
void Emulator::decode(uint32_t code, uint32_t wid, uint64_t uuid) {
  ...
  #endif
    case 3: {                 // DOT8 decoding on custom opcode
      switch (funct3) {
      case 0: { // DOT8
        auto instr = std::allocate_shared<Instr>(instr_pool_, uuid, FUType::ALU);
        instr->setOpType(AluType::DOT8);              // DOT8
        instr->setArgs(IntrAluArgs{0, 0, 0});         // no immediates / W
        instr->setDestReg(rd, RegType::Integer);      // rd
        instr->setSrcReg(0, rs1, RegType::Integer);   // rs1
        instr->setSrcReg(1, rs2, RegType::Integer);   // rs2
        ibuffer.push_back(instr);
      } break;
      default:
        std::abort();
